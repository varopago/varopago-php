...
class VaropagoApiConnector
{
    ...
    private function _request($method, $url, $params) {
        if (!class_exists('Varopago')) {
            throw new VaropagoApiError("Library install error, there are some missing classes");
        }
        VaropagoConsole::trace('VaropagoApiConnector @_request');

        $myId = Varopago::getId();
        if (!$myId) {
            throw new VaropagoApiAuthError("Empty or no Merchant ID provided");
        } else if (!preg_match('/^[a-z0-9]{20}$/i', $myId)) {
            throw new VaropagoApiAuthError("Invalid Merchant ID '".$myId."'");
        }

        $myApiKey = Varopago::getApiKey();
        if (!$myApiKey) {
            throw new VaropagoApiAuthError("Empty or no Private Key provided");
        } else if (!preg_match('/^sk_[a-z0-9]{32}$/i', $myApiKey)) {
            throw new VaropagoApiAuthError("Invalid Private Key '".$myApiKey."'");
        }

        $absUrl = Varopago::getEndpointUrl();
        if (!$absUrl) {
            throw new VaropagoApiConnectionError("No API endpoint set");
        }
        $absUrl .= '/'.$myId.$url;

        //$params = self::_encodeObjects($params);
        $headers = array('User-Agent: VaropagoPhp/v1');
    ...
    private function _curlRequest($method, $absUrl, $headers, $params, $auth = null) {
        VaropagoConsole::trace('VaropagoApiConnector @_curlRequest');
        ...
        if ($method == 'get') {
            ...
            throw new VaropagoApiError("Invalid request method '".$method."'");
        }
        ...
        VaropagoConsole::debug('Executing cURL: '.strtoupper($method).' > '.$absUrl);
        ...
        if ($rbody === false) {
            VaropagoConsole::error('cURL request error: '.curl_errno($curl));
        ...
        if (mb_detect_encoding($rbody, 'UTF-8', true) != 'UTF-8') {
            VaropagoConsole::warn('Response body is not an UTF-8 string');
        }

        VaropagoConsole::debug('cURL body: '.$rbody);
        VaropagoConsole::debug('cURL code: '.$rcode);
    ...
    private function encodeToQueryString($arr, $prefix = null) {
        ...
        $string = implode("&", $r);
        VaropagoConsole::debug('Query string: '.$string);
        return $string;
    }
    ...
    private function encodeToJson($arr) {
        ...
        VaropagoConsole::debug('JSON UTF8 string: '.$encoded);
        return $encoded;
    }

    private function interpretResponse($responseBody, $responseCode) {
        VaropagoConsole::trace('VaropagoApiConnector @interpretResponse');
        try {
            ...
            throw new VaropagoApiRequestError("Invalid response: ".$responseBody, $responseCode);
        }

        if ($responseCode < 200 || $responseCode >= 300) {
            VaropagoConsole::error('Request finished with HTTP code '.$responseCode);
    ...
    private function handleRequestError($responseBody, $responseCode, $traslatedResponse) {
        if (!is_array($traslatedResponse) || !isset($traslatedResponse['error_code'])) {
            throw new VaropagoApiRequestError("Invalid response body received from Varopago API Server");
        }
        ...
        switch ($responseCode) {
            ...
            case 401:
            case 403:
                throw new VaropagoApiAuthError($message, $error, $category, $request_id, $responseCode, $fraud_rules);
                break;
            ...
            case 500:
            case 503:
                throw new VaropagoApiRequestError($message, $error, $category, $request_id, $responseCode, $fraud_rules);
                break;
            ...
            case 412:
            case 423:
                throw new VaropagoApiTransactionError($message, $error, $category, $request_id, $responseCode, $fraud_rules);
                break;

            // Not Found
            default:
                throw new VaropagoApiError($message, $error, $category, $request_id, $responseCode, $fraud_rules);
        }
    }
    ...
    private function handleCurlError($errorCode, $message) {
        switch ($errorCode) {
            ...
            case CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Varopago.  Please check your internet connection and try again";
                break;
            default:
                $msg = "Unexpected error connecting to Varopago";
        }

        $msg .= " (Network error ".$errorCode.")";
        throw new VaropagoApiConnectionError($msg);
    }
    ...
    public static function request($method, $url, $params = null) {
        VaropagoConsole::trace('VaropagoApiConnector @request '.$url);
