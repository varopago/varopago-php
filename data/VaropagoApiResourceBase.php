 ...
 */
abstract class VaropagoApiResourceBase
{
    ...
    protected function __construct($resourceName, $params = array()) {
        ...
        if (!is_array($params)) {
            throw new VaropagoApiError("Invalid parameter type detected when instantiating an Varopago resource (passed '".gettype($params)."', array expected)");
        }
    ...
    protected static function getInstance($resourceName, $props = null) {
        VaropagoConsole::trace('VaropagoApiResourceBase @getInstance > '.$resourceName);
        if (!class_exists($resourceName)) {
            throw new VaropagoApiError("Invalid Varopago resource type (class resource '".$resourceName."' is invalid)");
        }
        if (is_string($props)) {
    ...
    private function processAttribute($k, $v) {
        VaropagoConsole::trace('VaropagoApiResourceBase @processAttribute > '.$k);
        $value = null;
        ...
        if ($this->isResource($resourceName)) {
            ...
            if ($this->isList($v)) {
                $list = VaropagoApiDerivedResource::getInstance($resourceName);
                $list->parent = $this;
    ...
    private function getResourceName($name) {
        VaropagoConsole::trace('VaropagoApiResourceBase @getResourceName');
        if (substr($name, 0, strlen('Varopago')) == 'Varopago') {
            return $name;
        }
        return 'Varopago'.ucfirst($name);
    }

    private function isResource($resourceName) {
        VaropagoConsole::trace('VaropagoApiResourceBase @isResource > '.$resourceName);

        return class_exists($resourceName);
    }

    private function registerInParent($resource) {
        VaropagoConsole::trace('VaropagoApiResourceBase @registerInParent');
        $parent = $this->parent;
        if ($parent instanceof VaropagoApiDerivedResource) {
        ...
        if ($container = $parent->getResource($resource->resourceName)) { // $resourceName
            if ($container instanceof VaropagoApiDerivedResource && method_exists($container, 'addResource')) {
                VaropagoConsole::trace('VaropagoApiResourceBase @registerInParent > registering derived resource in parent');
    ...
    private function getSerializeParameters() {
        VaropagoConsole::trace('VaropagoApiResourceBase @getSerializeParameters');
    ...
    protected function refreshData($data) {
        VaropagoConsole::trace('VaropagoApiResourceBase @refreshData');
        ...
        if (!is_array($data)) {
            throw new VaropagoApiError("Invalid data received for processing, cannot update the Varopago resource");
        }

        // unsets the unused attributes
        $removed = array_diff(array_keys($this->serializableData), array_keys($data));
        if (count($removed)) {
            VaropagoConsole::debug('VaropagoApiResourceBase @refreshData > removing unused data');
    ...
    protected function getResourceUrlName($pluralize = true) {
        ...
        if (substr($class, 0, strlen('Varopago')) == 'Varopago') {
            $class = substr($class, strlen('Varopago'));
        }
    ...
    protected function validateParams($params) {
        VaropagoConsole::trace('VaropagoApiResourceBase @validateParams');
        if (!is_array($params)) {
            throw new VaropagoApiRequestError("Invalid parameters type detected (type '".gettype($params)."' received, Array expected)");
        }
    }

    protected function validateId($id) {
        VaropagoConsole::trace('VaropagoApiResourceBase @validateId');
        if (!is_string($id) || !preg_match('/^[a-z][a-z0-9]{0,20}$/i', $id)) {
            throw new VaropagoApiRequestError("Invalid ID detected (value '".$id."' received, alphanumeric string not longer than 20 characters expected)");
        }
    }
    ...
    protected function _create($resourceName, $params, $props = null) {
        ...
        $response = VaropagoApiConnector::request('post', $resource->getUrl(), $params);
    ...
    protected function _retrieve($resourceName, $id, $props = null) {
        ...
        $response = VaropagoApiConnector::request('get', $resource->getUrl());
        return $resource->refreshData($response);
    }
    ...
    protected function _find($resourceName, $params, $props = null) {
        ...
        $list = array();
        $response = VaropagoApiConnector::request('get', $resource->getUrl(), $params);
    ...
    protected function _update() {
        ...
        if (count($params)) {
            $response = VaropagoApiConnector::request('put', $this->getUrl(), $params);
    ...
    protected function _updateCharge($params) {
        if (count($params)) {
            $response = VaropagoApiConnector::request('put', $this->getResourceUrl(), $params);
            return $this->refreshData($response);
        }
    }

    protected function _delete() {
        VaropagoApiConnector::request('delete', $this->getUrl(), null);
    ...
    protected function _getAttributes($param) {
        ...
        $response = VaropagoApiConnector::request('get', $url, null);
    ...
    public function getUrl() { // $includeId = true
        VaropagoConsole::trace('VaropagoApiResourceBase @getUrl > class/parent: '.get_class($this).'/'.($this->parent ? 'true' : 'false'));
        $parentUrl = '';

        if ($this->parent) {
            $parentUrl = $this->parent->getUrl();
            if ($this->parent instanceof VaropagoApiDerivedResource) {
    ...
    public function __set($key, $value) {
        VaropagoConsole::trace('VaropagoApiResourceBase @__set > '.$key.' = '.$value);
        if ($value === '' || !$value) {
            error_log("[Varopago Notice] The property '".$key."' will be set to en empty string which will be intepreted ad a NULL in request");
        }
        if (isset($this->$key) && is_array($value)) {
            ...
            throw new VaropagoApiError("The property '".$key."' cannot be assigned directly with an array");
    ...
    public function __get($key) {
        if (property_exists($this, $key)) {
            ...
            error_log("[Varopago Notice] Undefined property of $resourceName instance: $key"); // TODO error_log?
            return null;
        }
    }
